...
- **Rejected** items: do not fold them in.
  ...
  Record it in the Planning stage note under a `#### Deferred tidyings` heading, one line naming the file and the friction, so the `improvement-discovery` sweep can find it in a later improvement round.
