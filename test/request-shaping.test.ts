...
// ---------------------------------------------------------------------------
// Summarization transcript shaping (Issue #65)
//
// Pi's compaction, turn-prefix, and branch summarization paths transcribe the
// assistant's thinking blocks into a <conversation> envelope, which Anthropic's
// reasoning_extraction classifier refuses.  Shaping removes those paragraphs,
// but only on a payload that carries Pi's summarization system prompt.
// ---------------------------------------------------------------------------
...
function createSummarizationPayload(transcript: string = TRANSCRIPT) {
...
test("strips transcribed thinking from summarization payloads", () => {
  const shaped = shapeAnthropicOAuthPayload(
    createSummarizationPayload(),
  ) as ReturnType<typeof createSummarizationPayload>;
  const messages = shaped.messages as Array<{
    content: Array<{ text: string }>;
  }>;

  assert.equal(messages[0]?.content[0]?.text, STRIPPED_TRANSCRIPT);
});

test("leaves a conversation envelope alone without the summarization system prompt", () => {
  // A user may legitimately paste a Pi transcript into an ordinary turn; the
  // system-prompt gate is what keeps shaping off their message.
  const payload = createOAuthPayload({
    messages: [{ role: "user", content: [{ type: "text", text: TRANSCRIPT }] }],
  });
  ...
  assert.equal(messages[0]?.content[0]?.text, TRANSCRIPT);
});

test("derives the billing header hash from the stripped transcript", () => {
  // The strip must run before the billing header is built, or cch describes a
  // message body Anthropic never receives.
  const shaped = shapeAnthropicOAuthPayload(
    createSummarizationPayload(),
  ) as ReturnType<typeof createSummarizationPayload>;
  const systemBlocks = shaped.system as Array<{ text: string }>;
  ...
  assert.equal(
    systemBlocks[0]?.text,
    buildExpectedBillingHeader(STRIPPED_TRANSCRIPT),
  );
});
